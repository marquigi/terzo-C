...
export class PostsService {
  ...
  /**
 * Restituisce tutti i post (sincrono, dati locali).
 * In un'app reale questo metodo farebbe una chiamata HTTP e restituirebbe un Observable<Post[]>.
 */
  getPosts(): Post[] {
  ...
  /**
   * Restituisce la lista delle categorie dei post.
   * Ogni elemento ha:
   *  - id: slug usato per filtrare/eseguire query
   *  - title: etichetta leggibile per l'UI
   */
  getCategory(): PostCategory[] {
    // Anche questo, in produzione, partirebbe da un endpoint HTTP:
    // return this.http.get<postCategory[]>('/api/postCategories');
